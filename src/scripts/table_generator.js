function generateBadgeRating(rating) {
  ...
  if (rating) {
    if (rating < 2) {
      rating_badge_style = "badge-red badge-hover";
    } else if (rating < 4) {
      rating_badge_style = "badge-yellow badge-hover";
    } else {
      rating_badge_style = "badge-green badge-hover";
    }
  }
...
function generateBadgeRatingAmount(ratings_amount) {
  ...
  if (ratings_amount) {
    ...
    rating_badge_style = "bg-dark-subtle text-muted badge-hover";
  }
  let el = $(
    "<span class='badge " + rating_badge_style + "''>" + rating_text + "</span>"
  );
  return el;
}

// There is another function that is EXACTLY like this one. Find it in the other script
function isProfessorNameTBA(name) {
  if (!name) return false;
  return (
    name.toLowerCase().includes("announced") || name.toLowerCase() === "tba"
  );
}
...
function generateTableInContainerWithData(container, data) {
  ...
  const $table = $(
    '<table class="table betterknightsui-table table-bordered">'
  ).css({}).append(`
              <thead>
                  <tr>
                      <th scope="col">ID</th>
  ...
  $.each(data, function (index, item) {
    ...
    /// Class ID column
    const $classId = $(
      '<td style="text-align: center" data-order="' + item.class + '">'
    );
    if (item.class !== null) {
      ...
      if (_class_el) {
        ...
        $($(_class_el).find("a"))
          ...
          .addClass("font-betterknightsui-table");
    ...
    const $status = $(
      '<td style="text-align: center; " data-order="' + item.status + '">'
    ).html(
      `<span class="badge ${status_badge_style}" style="font-size: 0.7rem;">${status_badge_text}</span>`
    );
    ...
    if (
      ...
      sectionOptions.hasOwnProperty(item.section.type)
    ) {
    ...
    const $section = $(
      '<td style="text-align: center" data-order="' + item.section.type + '">'
    );
    ...
    const $mode = $(
      '<td data-order="' +
        item.mode +
        '" style="text-align: center; class="column">'
    ).append($mode_row);
    ...
    if (
      item.daysAndTimes &&
      item.daysAndTimes.length > 0 &&
      item.daysAndTimes[0].start_time
    )
      _startTime = item.daysAndTimes[0].start_time;
    const $daysAndTime = $(
      "<td data-order=" + getTimeSortingValue(_startTime) + "></td>"
    );
    $daysAndTime.css({
      "text-align": "center",
    });
    ...
    if (item.daysAndTimes && item.daysAndTimes.length > 0) {
      ...
      for (let i = 0; i < item.daysAndTimes.length; i++) {
        ...
        $timeRow.attr("title", "Duration: " + duration);
        ...
        $timeRow.append($dayBoxes).append($timeDiv).append($durationDiv);
        $daysAndTime.append($timeRow);
      }
    }
    ...
    if (item.instructors && item.instructors.length > 0) {
      for (let i in item.instructors) {
        ...
        let $badge_name = $(
          `<span class='badge ${
            isProfessorNameTBA(instructor_name)
    ...
    if (item.rooms) {
      for (let i in item.rooms) {
        ...
        if (item_room && item_room.google_maps_url) {
          let building = item_room.building
            ? item_room.building.replace(/&nbsp;/g, "").trim()
            : "";
          let room = item_room.room
            ? item_room.room
                .replace(/&nbsp;/g, "")
                .trim()
                .trim()
          ...
          let linkHtml = $(
            `<a href="${item_room.google_maps_url}" target="_blank" class="btn btn-link btn-sm">${building} ${room}</a>`
          );
    ...
    if (item.meetingDates && item.meetingDates !== null) {
      ...
      if (meeting_dates_text.includes("-")) {
        ...
        meeting_dates_html +=
          "<span style='display: block'>" + split[0].trim() + "</span>";
        meeting_dates_html +=
          "<span style='position: absolute; right: 20px; top: calc(50% - 9.5px);'>-</span>";
        meeting_dates_html +=
          "<span style='display: block'>" + split[1].trim() + "</span>";
      }
    }
    const $meetingDates = $('<td style="position: relative">').html(
      meeting_dates_html
    );
    ...
    if (item.elements && item.elements.buttonAction) {
      ...
      if (item.syllabusHref) {
        let $syllabus = $(
          '<a href="' +
            item.syllabusHref +
            '" class="btn btn-outline-primary btn-sm">Syllabus</a>'
        );
        $syllabus.css({
          "margin-top": "5px",
        });
  ...
  $.fn.dataTable.ext.order["dom-data-order"] = function (settings, col) {
    return this.api()
      ...
      .nodes()
      .map(function (td, i) {
        return $(td).attr("data-order") !== undefined
          ? $(td).attr("data-order")
          : $(td).text();
      });
  };
...
function updateAllElementsWithProfName(professorName, data) {
  ...
  for (i in elementsWithRating) {
    ...
    $($(newElement).closest("td")).attr(
      "data-order",
      parseInt((5 - rating) * 100)
    );
    ...
    if (container) {
      ...
      $(container)
        .css("cursor", "pointer")
        .attr("title", "Click to go to www.ratemyprofessors.com");
  ...
  for (i in elementsWithRatingAmount) {
    ...
    let newElement = $(generateBadgeRatingAmount(rating_amount)).insertAfter(
      oldElement
    );
    oldElement.remove();
  }
